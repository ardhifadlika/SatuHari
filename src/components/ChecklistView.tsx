  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-[#0D1C17]">Checklist Persiapan</h2>
          <p className="text-sm text-[#788A82]">Susun prioritas, jadwalkan, dan bagi tugas persiapan pernikahan bersama pasangan.</p>
        </div>
        <button
      ...
      <div className="p-4 bg-[#FAF5F5] rounded-2xl border border-[#E8DDD9] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xs">
        <div className="space-y-1">
          <span className="text-xs font-bold text-[#1C3E33] flex items-center font-mono uppercase tracking-wider">
            <Wand2 className="w-3.5 h-3.5 mr-1.5 text-[#BA3444]" />
            Asisten Template Rencana
          </span>
          <p className="text-xs text-[#788A82]">Butuh panduan? Tambahkan tugas rekomendasi pernikahan Indonesia berdasarkan jangka waktu sisa Anda.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5">
          <button
            ...
            className="px-3 py-1.5 bg-white hover:bg-[#F2E9E8] border border-[#E8DDD9] hover:border-[#1C3E33] text-[#0D1C17] font-semibold rounded-xl text-xs transition inline-flex items-center cursor-pointer shadow-2xs"
          >
            🗓️ Rencana 12 Bulan
          </button>
          <button
      ...
      <div className="bg-white p-4 rounded-xl border border-[#E8DDD9] shadow-xs space-y-3">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex items-center space-x-2">
            <span className="text-xs text-[#788A82] font-medium whitespace-nowrap inline-flex items-center"><User className="w-3.5 h-3.5 mr-1" /> PIC:</span>
            <select
          ...
          {/* Status Filter */}
          <div className="flex items-center space-x-2">
            <span className="text-xs text-[#788A82] font-medium whitespace-nowrap inline-flex items-center"><Filter className="w-3.5 h-3.5 mr-1" /> Status:</span>
            <select
            ...
            >
              ...
              <option value="NOT_STARTED">Belum Dimulai (Warn)</option>
              <option value="IN_PROGRESS">Sedang Dikerjakan (Info)</option>
              <option value="COMPLETED">Telah Selesai (Success)</option>
      ...
      {filteredTasks.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#E8DDD9] p-12 text-center max-w-xl mx-auto space-y-3">
          ...
          <p className="text-sm font-semibold text-[#0D1C17]">Tidak ada tugas persiapan pernikahan.</p>
          <p className="text-xs text-[#788A82]">Sesuaikan filter pencarian, gunakan asisten rencana, atau buat rencana tugas perdana Anda.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredTasks.map((task) => (
            <div 
              key={task.id}
              className={`bg-white border rounded-2xl p-5 shadow-xs transition hover:shadow-sm flex flex-col justify-between space-y-4 ${task.status === 'COMPLETED' ? 'border-[#E8DDD9]/50 bg-[#FAF5F5]/60 opacity-85' : 'border-[#E8DDD9]'}`}
            >
              <div className="space-y-2">
                <div className="flex justify-between items-start gap-2">
                  <h3 className={`text-sm font-bold text-[#0D1C17] ${task.status === 'COMPLETED' ? 'line-through text-[#788A82]' : ''}`}>
                    {task.title}
                  </h3>
                  
                  {/* Status Badges Select maintaining semantic color system: Green=Success, Blue=Info, Yellow=Warn, Red=Danger */}
                  <select
                ...
                <p className={`text-xs text-[#2D3D36] line-clamp-2 ${task.status === 'COMPLETED' ? 'text-[#788A82]' : ''}`}>
                  {task.description || 'Tidak ada deskripsi rinci.'}
                </p>
              </div>

              {/* Meta & Actions */}
              <div className="pt-3 border-t border-[#E8DDD9]/60 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-[#788A82] font-mono">
                <div className="flex flex-wrap gap-x-3 gap-y-1 inline-flex items-center">
                  <span className="flex items-center">
                    <Calendar className="w-3.5 h-3.5 mr-1 text-[#788A82]" />
                    {new Date(task.dueDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                  </span>
                  
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${task.owner === 'PARTNER_A' ? 'bg-[#BA3444]/15 text-[#BA3444]' : task.owner === 'PARTNER_B' ? 'bg-[#1C3E33]/15 text-[#1C3E33]' : 'bg-[#487A64]/15 text-[#487A64]'}`}>
                    PIC: {task.owner === 'PARTNER_A' ? 'Ami' : task.owner === 'PARTNER_B' ? 'Ardhi' : 'Kombinasi'}
                  </span>

                  {task.attachmentName && (
                    <span 
                      className="bg-[#FAF5F5] border border-[#E8DDD9] px-1.5 py-0.5 rounded text-[9px] text-[#2D3D36] truncate flex items-center max-w-40"
                ...
                <div className="flex items-center space-x-1.5 self-end sm:self-auto">
                  <button
                    ...
                    className="p-1 text-[#788A82] hover:text-[#1C3E33] rounded hover:bg-[#F2E9E8] transition cursor-pointer border-0"
                  ...
                  <button
                    ...
                    className="p-1 text-zinc-400 hover:text-red-600 rounded hover:bg-red-50 transition cursor-pointer border-0"
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-[#0D1C17]/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6 border border-[#E8DDD9] relative overflow-hidden">
            <h3 className="text-lg font-bold text-[#0D1C17] mb-1">Tambah Tugas Pernikahan</h3>
            <p className="text-xs text-[#788A82] mb-4">Pastikan PIC dan tenggat pengerjaan sudah ideal bersama calon pendamping.</p>

            <form onSubmit={handleAddNewTask} className="space-y-3 text-xs font-semibold text-[#0D1C17]">
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 border border-[#E8DDD9] rounded-xl text-xs bg-white focus:outline-none focus:ring-2 focus:ring-[#1C3E33]/20 focus:border-[#1C3E33]"
                  >
              ...
              <div className="flex space-x-2 pt-4 border-t border-[#E8DDD9] text-xs font-semibold">
                <button
                  ...
                  className="flex-1 py-2.5 border border-[#E8DDD9] hover:bg-[#FAF5F5] rounded-xl transition text-[#0D1C17] cursor-pointer bg-white"
                >
                  Kembali
                </button>
                <button
